...
public class FelixResolverState implements Resolver.ResolverState
{
    ...
    {
        List<Module> fragments = host.getFragments();
...
//       attaching with updated modules.
            if (hostCap.getModule().isResolved()
                || hostCap.getModule().isStale()
                || hostCap.getModule().isRemovalPending())
            {
                it.remove();
            }
...
//       attaching with updated modules.
                ...
                if (!f.isStale() && !f.isRemovalPending())
                {
                    fragment = f;
                }
            }
